...
export const CalendarView: React.FC = () => {
  ...
  // Rule of Hooks fix: Initialize viewMode using a function to avoid conditional hooks.
  // Defaults to '3-day' view on mobile screens for cleaner presentation.
  const [viewMode, setViewMode] = useState<ViewMode>(() =>
    window.matchMedia("(max-width: 768px)").matches ? '3-day' : 'month'
  );
  ...
  return (
    <div className="animate-in fade-in zoom-in duration-500 h-full flex flex-col min-h-full">
      ...
      {/* 
        ...
        Sets `overflow-x-auto` on the container and forces a minimum width of 768px for Month/Week
        views, ensuring that layout columns do not shrink past usability limits on mobile screens.
      */}
      <div className="flex-1 glass-panel rounded-2xl border border-secondary/20 flex flex-col min-h-[600px] shadow-sm overflow-x-auto">
        <div className={`flex-1 flex flex-col ${viewMode !== '3-day' ? 'min-w-[768px]' : 'min-w-0'}`}>
          {/* Dynamic Days Header */}
